...
const showData = (datas) => {
  ...
  datas.forEach((data, idx) => {
    ...
    numId.appendChild(document.createTextNode(idx + 1));
    row.appendChild(numId);

    for (let keys in data) {
      const cell = document.createElement("td");
      // cell.appendChild(document.createTextNode(data.keys));
      cell.appendChild(document.createTextNode(data[keys]));
...
searchBtn.addEventListener("click", (e) => {
  ...
  filterData(data, searchInput.value);
});

//trigger enter to search data
